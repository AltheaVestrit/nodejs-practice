...
const validateUser = [
    ...
    body("bio").trim()
        .isLength({ max: 200 }).withMessage('Bio must be max 200 characters long.')
        .optional({ values: "falsy" }),
];
...
exports.usersSearchGet = (req, res) => {
    const foundUser = usersStorage.findUser(req.query.name, req.query.email);
    if (foundUser) {
        res.redirect(`/${foundUser.id}/update`);
    } else {
